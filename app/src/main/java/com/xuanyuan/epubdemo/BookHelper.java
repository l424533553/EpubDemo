...
public class BookHelper {
     ...
     */
    @NonNull
    public BookBean parseBook(@NonNull Book book) throws IOException {
        /* *********************************/
...
//            if (coverImage!=null) {
//               binding.image .setImageBitmap(coverImage);
//            }else {
...
//            bookBean.setBitmap(BitmapFactory.decodeByteArray(data, 0, data.length));
    ...
    private List<EpubBean> parseHtml(String strHtml) {
        ...
        Log.i("111111111111", Arrays.toString(list.toArray()));
    ...
    @NonNull
    public InputStream getInputStream(@NonNull Context context, long index) throws IOException {
        bookId = index;
        InputStream in;
        switch ((int) index) {
            case 4:
